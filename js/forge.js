...
function showCraftItemDetails(index) {
    ...
    if (ranges) {
        ...
        if (item.type === 'weapon') {
            ...
            statsHtml += `<div class="stat-line"><span class="stat-label">Nhanh nhẹn</span><span class="stat-value">${fmt(agi[0],1)} ~ ${fmt(agi[1],1)}</span></div>`;
        ...
        <div class="item-detail">
            ...
            <div class="item-header">
                <div class="item-basic-info">
                    <div class="item-name" style="color:${window.qualityColors ? (window.qualityColors[item.quality] || '#fff') : '#fff'}">${displayName}</div>
                    ...
                    <div class="info-line"><span class="info-label">Tinh Luyện:</span><span class="info-value">${enhanceLevel} lần</span></div>
...
function craftItem(itemType, itemQuality, itemIndex) {
    ...
    if (typeof window.updateForgeResources === 'function') {
        window.updateForgeResources();
    }
}
...
function updateForgeResources() {
    const gameState = ensureGameState();
    const grid = document.getElementById('resources-grid');
    if (!gameState || !grid) return;
    grid.innerHTML = `
        <div class="resource-item">
            <img src="images/vang.png" alt="Vàng" class="resource-icon" onerror="this.src='images/placeholder.png'">
            <span>${gameState.gold}</span>
        </div>
        <div class="resource-item">
            <img src="images/linh_thach.png" alt="Linh Thạch" class="resource-icon" onerror="this.src='images/placeholder.png'">
            <span>${gameState.spiritStones}</span>
        </div>
        <div class="resource-item">
            <img src="images/huyen_thiet.png" alt="Huyền Thiết" class="resource-icon" onerror="this.src='images/placeholder.png'">
...
if (typeof window !== 'undefined') {
    window.updateForgeResources = updateForgeResources;
}
