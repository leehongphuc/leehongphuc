...
function showCraftItemDetails(index) {
	...
	if (ranges) {
		...
		if (item.type === 'weapon') {
			statsText += `<div class="stat-line">Vũ khí  sát thương VL/Phép thuật từ ${fmtComma(dmgMin, 0)}${dash}${fmtComma(dmgMax, 0)}   chí mạng ${fmtComma(critMin, 0)}% ~ ${fmtComma(critMax, 0)}%  tốc độ ${fmtComma(spdMin, 1)}${dash}${fmtComma(spdMax, 1)}</div>`;
		} else if (item.type === 'armor') {
			statsText += `<div class="stat-line">Giáp Máu ${fmtComma(hpMin, 0)}${dash}${fmtComma(hpMax, 0)} phòng thủ vật lý ${fmtComma(pdefMin, 0)}% ~ ${fmtComma(pdefMax, 0)}%, phòng thủ phép thuật ${fmtComma(mdefMin, 0)}% ~ ${fmtComma(mdefMax, 0)}%</div>`;
