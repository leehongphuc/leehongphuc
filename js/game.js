...
function showItemDetails(index, isEquipped = false, fromCharacterEquip = false) {
    ...
    if (item.type !== 'giftBag' && item.type !== 'material') {
        statsText = Object.entries(item.stats).map(([stat, value]) => {
            const statName = stat === 'physicalDamage' ? 'Sát thương vật lý' :
                stat === 'magicDamage' ? 'Sát thương phép thuật' :
            ...
            const displayValue = `${value}${['criticalChance', 'physicalDefense', 'magicDefense'].includes(stat) ? '%' : stat === 'agility' ? '' : ''}`;
            return `<div class="stat-line"><span class="stat-label">${statName}</span><span class="stat-value">${displayValue}</span></div>`;
        }).join('');
    }
    let actionButton = '';
    ...
    // Tạo tên hiển thị với số lần cường hóa
    const enhanceLevel = item.enhanceLevel || 0;
    const displayName = item.type !== 'giftBag' && item.type !== 'material' 
        ? `${item.name} +${enhanceLevel}` 
        : item.name;
    ...
    modalContent.innerHTML = `
        ...
        <div class="item-detail">
            ...
            <div class="item-info">
                <div class="item-name" ${qualityStyle}>${displayName}</div>
<div class="info-line"><span class="info-label">Cấp bậc:</span><span class="info-value">${item.tier}</span></div>
<div class="info-line"><span class="info-label">Phẩm chất:</span><span class="info-value" ${qualityStyle}>${item.quality}</span></div>
${item.type !== 'giftBag' && item.type !== 'material' ? `<div class="info-line"><span class="info-label">Tinh Luyện:</span><span class="info-value">${enhanceLevel} lần</span></div>` : ''}
        ...
        if (item && item.equipped && item.name) {
            ...
            equipmentItem.innerHTML = `
                ...
                <div class="equipment-info">
                    <div class="equipment-type" ${qualityStyle}>${`${item.name} +${item.enhanceLevel || 0}`}</div>
            ...
            .forEach((item, index) => {
                ...
                inventoryItem.innerHTML = `
                    ...
                    <div class="inventory-info">
                        <div class="inventory-name" ${qualityStyle}>${item.type !== "giftBag" && item.type !== "material" ? `${item.name} +${item.enhanceLevel || 0}` : item.name}</div>
