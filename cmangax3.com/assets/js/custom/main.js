function transfer_data(data) {
    $.post("../assets/inc/firebase/transfer_data.php", {
            data: data
        })
        .done(function(data) {});
}
...
function time_convert(time_send) {
    ...
    target_zone_time = target_zone_time.toLocaleString('en-US', {
...
function get_account_secret(account, target) {
    return new Promise(callback => {
        $.post("/assets/ajax/" + target + ".php", {
                action: 'get_account_secret',
                account_id: account
            })
            .done(function(data) {
...
function chapter_load_page() {
    if ($('.chapter_non_load').length > 0) {
        ...
        if (image_error_list["dc1.mideman.com"] > 0 && image_error_list["s3.mideman.com"] == 0) {
            ...
            $.post("/assets/ajax/user.php", {
                action: 'chapter_repair',
                chapter_id: chapter_id
            }, function(data) {
                chapter_image_render(1)
            });
        }
    }
}
...
function processChapterText(text) {
    ...
    return text.replace(regex, function(match, capturedText) {
        ...
        return `<img src="/data-image/text.php?text=${encodedText}&w=${width}&size=${font_size}&line_height=${line_height}&color=${color}" />`;
    });
}
...
async function chapter_image_render(version = 0) {
    ...
    if (chapter_content.status == 1) {
        ...
        if (album_file == "image") {
            ...
            if (chapter_content.audio) {
                audioImport(chapter_content.audio, 'Chapter ' + chapter_info.num);
            }
        }
    ...
    $(document).ready(function() {
        const observerOptions = {
            ...
            threshold: Array.from({
                length: 11
            }, (_, i) => i / 10) // Các mốc từ 0.1 đến 1.0
        };
    ...
    if (album_file == "text") {
        var text_setting = getSetting('text_setting') ? ? null;
        if (text_setting) {
            $('.chapter_content .chapter_text').attr('style', `font-family: ${text_setting.font_family};font-size: ${text_setting.font_size}px;line-height: ${text_setting.line_height}px;color: ${text_setting.color};background-color: ${text_setting.background_color};text-align: ${text_setting.text_align};`);
        }
...
function chapter_ads() {
    ...
    setTimeout(() => {
        var adsView = getCookie('adsView') ? ? "no";
        if (adsView == "no") {
...
function comment_delete_all(user_id) {
    alertify.confirm('Bạn có chắc chắn xóa toàn bộ bình luận của người này', function() {
        ...
        ok: language_text('text_yes'),
        cancel: language_text('text_no')
    });
}
...
function activity_position_select(e) {
    ...
    $.post('assets/ajax/character_activity.php', {
        action: "activity_position_select",
        type: type,
        target: target,
        position: popup_data.target,
        activity: activity_position_data.activity
    }, function(data) {
        $('#result').html(data);
    });
}
